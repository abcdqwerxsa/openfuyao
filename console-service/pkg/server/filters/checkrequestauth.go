...
func (rc *requestAuthChecker) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil && err.Error() == constant.OnlyAccessTokenExpiredErrorStr {
		zlog.Info("rotating access-token")
	...
	if err != nil {
		if isFuyaoApi(req) {
			...
			// save original URL to cookie before redirect to login
			// For API requests (XHR/Fetch), use Referer header to get the original page URL
			referrerURL := getReferrerURL(req)
			setReferrerCookie(w, referrerURL)
			// redirect to log in for other requests
			prefix := req.Header.Get("X-Forwarded-Prefix")
			http.Redirect(w, req, prefix+path.Join(runtime.RestRootPath, "/auth/login"), http.StatusSeeOther)
		}
		return
	}
...
// setReferrerCookie sets a cookie to save the original request URL before OAuth redirect
func setReferrerCookie(w http.ResponseWriter, originalURL string) {
	cookie := http.Cookie{
		Name:     constant.CookieNameReferrer,
		Value:    originalURL,
		Path:     "/",
		HttpOnly: true,
		Secure:   true,
		SameSite: http.SameSiteLaxMode,
		Expires:  time.Now().Add(defaultCookieExpireTime * time.Second),
	}
	http.SetCookie(w, &cookie)
}

// getReferrerURL returns the appropriate referrer URL for redirect after login.
// If Referer header is present (e.g., from XHR/Fetch requests), use it to get the original page URL.
// Otherwise, use the request URL (for direct page access).
func getReferrerURL(req *http.Request) string {
	// For AJAX requests, the Referer header contains the page URL
