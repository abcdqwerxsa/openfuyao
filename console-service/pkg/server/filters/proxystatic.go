...
func (sp staticProxy) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	// Capture the forwarded prefix before proxying (it may not be preserved in the response)
	forwardedPrefix := req.Header.Get("X-Forwarded-Prefix")

	sp.proxy.ModifyResponse = func(resp *http.Response) error {
		if forwardedPrefix == "" {
			return nil
		}
		...
		scriptTag := fmt.Sprintf("<script>window.__CONSOLE_BASE_PATH__ = %q;</script>", forwardedPrefix)
		injected := strings.Replace(string(body), "</head>", scriptTag+"</head>", 1)
