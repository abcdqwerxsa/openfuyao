...
export default function RoleBindCreate() {
  ...
  const handleSaveYaml = async () => {
    ...
    if (!errorCount) {
      ...
      if (passNamespace) {
        try {
          const res = await addRoleBindingsYamlData(passNamespace, yamlJson);
        ...
        } catch (roleBindCreateError) {
          if (roleBindCreateError.response.status === ResponseCode.Forbidden) {
            forbiddenMsg(messageApi, roleBindCreateError);
          } else {
            messageApi.error(`创建失败！${roleBindCreateError.response.data.message}`);
          }
        }
      } else {
  ...
  const handleResetCode = () => {
    setRoleBindYaml(roleBindYamlExample);
    childCodeMirrorRef.current.resetCodeEditor(roleBindYamlExample);
  };

  const exportYaml = () => {
    exportYamlOutPut('RoleBinding', roleBindYaml);
      ...
      <div className="yaml_card">
        <div className="yaml_flex_box">
          ...
          <div className="yaml_tools">
            ...
            <div className="tool_word_group RoleBindCreate" onClick={handleCopyYaml}>
