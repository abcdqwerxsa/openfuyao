...
import SecretInfo from '@/pages/container/configuration/secret/detail/SecretInfo';
import SecretYaml from '@/pages/container/configuration/secret/detail/SecretYaml';
...
export default function SecretDetail() {
  ...
  const [secretDetailTabKey, setSecretDetailTabKey] = useState(activeKey || 'info');
  ...
  const getSecretDetailsInfo = useCallback(async () => {
    if (namespace && name) {
      ...
      const res = await getSecretDetails(namespace, name);
  ...
  const handleAnnotationOk = async (data) => {
    ...
    try {
      const res = await updateSecretLabelAnnotation(namespace, addAnnotationList, name);
      if (res.status === ResponseCode.OK) {
  ...
  const handleLabelOk = async (data) => {
    ...
    try {
      const res = await updateSecretLabelAnnotation(namespace, addLabelList, name);
      if (res.status === ResponseCode.OK) {
  ...
  const handleDelConfirm = async () => {
    try {
      const res = await deleteSecret(namespace, name);
      if (res.status === ResponseCode.OK) {
  ...
  useEffect(() => {
    if (secretDetailTabKey === 'info' || secretDetailTabKey === 'yaml') {
      getSecretDetailsInfo();
    }
  }, [getSecretDetailsInfo]);
  ...
  return (
    <div className='child_content withBread_content SecretDetail'>
      ...
      >
        ...
        {detailLoaded && (
          <AnnotationModal
            ...
            type="annotation"
            dataList={secretData?.metadata?.annotations}
        ...
        {detailLoaded && (
          <AnnotationModal
            ...
            type="label"
            dataList={secretData?.metadata?.labels}
      ...
      {detailLoaded && (
        <Tabs
          items={items}
          activeKey={secretDetailTabKey}
          onChange={handleSecretDetailIndex}
