...
import { updateLimitRange } from '@/api/containerApi';
export default function LimitRangeDetailYaml({ limitRangeYamlProps, readOnly, handleEditFn, refreshFn }) {
  const [limitRangeYaml, setLimitRangeYaml] = useState(limitRangeYamlProps);
  ...
  const handleSaveYaml = async () => {
    ...
    let passName = '';
    try {
      yamlJson = yamlTojson(limitRangeYaml);
      passNamespace = yamlTojson(limitRangeYamlProps).metadata.namespace; // 默认取之前的命名空间
      passName = yamlTojson(limitRangeYamlProps).metadata.name;
    } catch (e) {
    ...
    if (!errorCount) {
      if (passNamespace) {
        try {
          const res = await updateLimitRange(passNamespace, passName, yamlJson);
  ...
  const handleResetCode = () => {
    setLimitRangeYaml(limitRangeYamlProps);
    childCodeMirrorRef.current.resetCodeEditor(limitRangeYamlProps);
  };

  const exportYaml = () => {
    exportYamlOutPut(`${fileName}`, limitRangeYaml);
  ...
  useEffect(() => {
    const detailData = yamlTojson(limitRangeYamlProps);
  ...
  return <div className="tab_container container_margin_box normal_container_height">
    ...
    <div className="yaml_card">
      <div className="yaml_flex_box">
        ...
        <div className="yaml_tools">
          <div className="tool_word_group limitRangeDetailYamlExport" onClick={exportYaml}>
            <ExportOutlined className="common_antd_icon primary_color" />
            <span>导出</span>
          </div>
          <div className="tool_word_group limitRangeDetailYamlCopy" onClick={handleCopyYaml}>
    ...
    <div className="yaml_space_box">
      <CodeMirrorEditor className={readOnly ? 'no_btn' : ''} yamlData={limitRangeYaml} changeYaml={handleChangeYaml} isEdit={!readOnly} ref={childCodeMirrorRef} />
