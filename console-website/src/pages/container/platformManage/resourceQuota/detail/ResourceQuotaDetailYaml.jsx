...
import { updateResourceQuota } from '@/api/containerApi';
import CodeMirrorEditor from '@/components/CodeMirrorEditor';
import { yamlTojson, exportYamlOutPut, forbiddenMsg } from '@/tools/utils';
export default function ResourceQuotaDetailYaml({ resourceQuotaYamlProps, readOnly, handleEditFn, refreshFn }) {
  const [resourceQuotaYaml, setResourceQuotaYaml] = useState(resourceQuotaYamlProps);
  ...
  const handleSaveYaml = async () => {
    ...
    try {
      ...
      passNamespace = yamlTojson(resourceQuotaYamlProps).metadata.namespace; // 默认取之前的命名空间
      passName = yamlTojson(resourceQuotaYamlProps).metadata.name;
    } catch (e) {
    ...
    if (!errorCount) {
      if (passNamespace) {
        try {
          const res = await updateResourceQuota(passNamespace, passName, yamlJson);
  ...
  const handleResetCode = () => {
    setResourceQuotaYaml(resourceQuotaYamlProps);
    childCodeMirrorRef.current.resetCodeEditor(resourceQuotaYamlProps);
  };

  const exportYaml = () => {
    exportYamlOutPut(`${fileName}`, resourceQuotaYaml);
  ...
  useEffect(() => {
    const detailData = yamlTojson(resourceQuotaYamlProps);
    setFileName(detailData.metadata.name);
  }, [resourceQuotaYamlProps]);
  ...
  return <div className="tab_container container_margin_box normal_container_height">
    ...
    <div className="yaml_space_box">
      <CodeMirrorEditor className={readOnly ? 'no_btn' : ''} yamlData={resourceQuotaYaml} changeYaml={handleChangeYaml} isEdit={!readOnly} ref={childCodeMirrorRef} />
