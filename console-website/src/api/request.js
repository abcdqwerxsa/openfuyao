    ...
    ir({
      ...
      .catch(error => {
        ...
        if (url !== '/password/modify' && code === ResponseCode.UnAuthorized) {
          ...
          logout().then(() => {
            if (window.location.pathname !== withBasePath('/rest/auth/login') || !window.location.pathname.includes('/auth/login')) {
              localStorage.clear();
...
export function DELETE(url, data, ...args) {
  return request('delete', url, data, ...args);
}
